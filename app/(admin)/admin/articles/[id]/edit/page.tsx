...
import { getArticleByIdService, updateArticleService, UpdateArticleDTO } from "@/lib/api/articles/services";
...
function ArticleEditorPageContent() {
    ...
    const { data: article, isLoading: loadingArticle } = useQuery({
        ...
        queryFn: () => getArticleByIdService(articleId),
        enabled: !!articleId,
    });
    ...
    // Pre-fill form when article data is loaded
    useEffect(() => {
        if (article) {
            setForm({
                title: article.title || "",
